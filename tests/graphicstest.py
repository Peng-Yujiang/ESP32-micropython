...
import time
import math
spi = SPI(2, baudrate=32000000, polarity=0, phase=0, sck=Pin(14), mosi=Pin(13), miso=Pin(12)) # DC/A0, Reset, CS
...
def tftprinttest():
    tft.fill('Black');
    v = 30
    tft.text((0, v), "Hello World!", 'Red', sysfont, 1, nowrap=True)
    v += sysfont["Height"]
    tft.text((0, v), "Hello World!", 'Yellow', sysfont, 2, nowrap=True)
    v += sysfont["Height"] * 2
    tft.text((0, v), "Hello World!", 'Green', sysfont, 3, nowrap=True)
    v += sysfont["Height"] * 3
    tft.text((0, v), str(1234.567), 'Blue', sysfont, 4, nowrap=True)
    time.sleep_ms(1500)
    tft.fill('Black');
    v = 0
    tft.text((0, v), "Hello World!", 'Red', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), str(math.pi), 'Green', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), " Want pi?", 'Green', sysfont)
    v += sysfont["Height"] * 2
    tft.text((0, v), hex(8675309), 'Green', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), " Print HEX!", 'Green', sysfont)
    v += sysfont["Height"] * 2
    tft.text((0, v), "Sketch has been", 'White', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), "running for: ", 'White', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), str(time.ticks_ms() / 1000), 'Purple', sysfont)
    v += sysfont["Height"]
    tft.text((0, v), " seconds.", 'White', sysfont)

def test_main():
    tft.fill('Black')
    tft.text((0, 0), "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur adipiscing ante sed nibh tincidunt feugiat. Maecenas enim massa, fringilla sed malesuada et, malesuada sit amet turpis. Sed porttitor neque ut ante pretium vitae malesuada nunc bibendum. Nullam aliquet ultrices massa eu hendrerit. Ut sed nisi lorem. In vestibulum purus a tortor imperdiet posuere. ", 'White', sysfont, 1)
    time.sleep_ms(1000)

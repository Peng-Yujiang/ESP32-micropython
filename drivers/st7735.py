...
class TFT(object) :
  ...
  _TFTBGR = 0x08 #When set color is bgr else rgb.
  _TFTRGB = 0x00
  ...
  color_dic = {'Black':BLACK, 'Red':RED, 'Maroon':MAROON,
                'Green':GREEN, 'Forest':FOREST, 'Blue':BLUE,
                 'Navy':NAVY, 'Cyan':CYAN, 'Yellow':YELLOW,
                 'Purple':PURPLE, 'White':WHITE, 'Gray':GRAY}
  ...
  def __init__( self, spi, aDC, aReset, aCS) :
    ...
    self._size = TFT.SCREEN_SIZE
    self._offset = bytearray([0,0])
    self.rotate = 2                   #Vertical with bottom toward pins.
  ...
  def invertcolor( self, aBool  = True) :
  ...
  def text( self, aPos, aString, aColor, aFont = sysfont, aSize = 1, nowrap = False ) :
  ...
  def char( self, aPos, aChar, aColor, aFont = sysfont, aSizes = (1, 1)) :
        ...
        for q in range(fontw) :
          ...
          for r in range(fonth) :
            if c & 0x01 :
              pos = 2 * (r * fontw + q)
              if type(aColor) == str:
                aColor = TFT.color_dic.get(aColor)
              buf[pos] = aColor >> 8
...
#   @micropython.native
  ...
  def _setMADCTL( self ) :
    ...
    rgb = TFT._TFTRGB if self._rgb else TFT._TFTBGR
    self._writedata(bytearray([TFT._TFTRotations[self.rotate] | rgb]))
  ...
  def initb( self ) :
    ...
    self._size = (TFT.SCREEN_SIZE[0] + 2, TFT.SCREEN_SIZE[1] + 1)
    self._reset()
